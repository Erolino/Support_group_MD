...
if 1 == 0:
    with codecs.open(joinp(process_path,'unigram_sentences_all.txt'), 'w', encoding='utf_8') as f:
        for sentence in lemmatized_sentence_corpus(A):
            f.write(sentence + '\n')

unigram_sentences = LineSentence(joinp(process_path,'unigram_sentences_all.txt'))
...
if 1 == 0:
    ...
    bigram_model.save(joinp(process_path, 'bigram_model_all'))
    
# load the finished model from disk
bigram_model = Phrases.load(joinp(process_path, 'bigram_model_all'))

### fitting the unigram text to the bigram "Model" (and writing it to a file)
if 1 == 0:

    with codecs.open(joinp(process_path,'bigram_sentences_all.txt'), 'w', encoding='utf_8') as f:
        for unigram_sentence in unigram_sentences: 
            bigram_sentence = u' '.join(bigram_model[unigram_sentence])
            f.write(bigram_sentence + '\n')

bigram_sentences = LineSentence(joinp(process_path,'bigram_sentences_all.txt'))
...
if 1 == 0:
    ...
    trigram_model.save(joinp(process_path,'trigram_model_all.txt'))
    
# load the finished model from disk
trigram_model = Phrases.load(joinp(process_path,'trigram_model_all.txt'))

### fitting the unigram text to the bigram "Model" (and writing it to a file)
if 1 == 0:

    with codecs.open(joinp(process_path,'trigram_sentences_all.txt'), 'w', encoding='utf_8') as f:
        for bigram_sentence in bigram_sentences: 
            trigram_sentence = u' '.join(trigram_model[bigram_sentence])
            f.write(trigram_sentence + '\n')

trigram_sentences = LineSentence(joinp(process_path,'trigram_sentences_all.txt'))
...
if 1 == 0:
    ...
    with codecs.open(joinp(process_path,'trigram_trans_sents_all.txt'), 'w', encoding='utf_8') as f:
...
trigram_transformed=LineSentence(join(process_path,'trigram_trans_sents_all.txt'))
  
'''  
    with codecs.open(joinp(process_path,'trigram_transformed_sents_all.txt'), 'w', encoding='utf_8') as f:
